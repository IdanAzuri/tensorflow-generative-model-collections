...
class CNNClassifier():
	...
	def save_and_plot_results_cv(self, name_of_measure, array, color="b", marker="P", dir="classifier_results_seed_{}/".format(SEED)):
		name_figure = dir + "classifier_MMinfoGAN_{}_{}".format(self.fname, name_of_measure)
		pickle.dump(array, open("{}.pkl".format(name_figure), 'wb'))
...
def main():
	...
	if train_classifier_for_generated_data:
		...
		for i in range(cv):
			...
			X_test = X_test_real  # np.append(X_test_real,X_test).reshape(-1,784)
			y_test = y_test_real  # np.append(y_test_real.reshape(-1,10),y_test).reshape(-1,10)
