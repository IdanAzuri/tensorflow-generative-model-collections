...
class CNNClassifier():
    ...
    def set_dataset(self, training, labels):
        training, labels = shuffle(training, labels, random_state=SEED)
        self.train_X = np.asarray(training[1000:]).reshape(-1, 784)
        self.train_y = np.asarray(labels[1000:]).reshape(-1, 10)
        self.test_X = self.train_X.reshape(-1, 784)
    ...
    def _create_model(self):
        ...
        self.l2_regularization = self.lamb * tf.nn.l2_loss(self.W_conv1) + self.lamb * tf.nn.l2_loss(
            self.W_conv1) + self.lamb * tf.nn.l2_loss(
        ...
        for epoch in range(self.num_epochs):
            for i in range(start_batch_id, self.num_batches):
                ...
                if i % self.num_batches - 1 == 0:
                    ...
                    accuracy, confidence, loss = self.test(self.test_X.reshape(-1, 784), self.test_y.reshape(-1, 10),
                                                           epoch * i)
                    ...
                    if not use_confidence:
                        self.train_step.run(session=self.sess, feed_dict={self.x: X_batch, self.y_: y_batch,
                        ...
                        if len(high_confidence_threshold_indices[high_confidence_threshold_indices]) > 0:
                            _ = self.sess.run([self.train_step],
                                              feed_dict={self.x: X_batch[high_confidence_threshold_indices],
                                                         self.y_: y_batch[high_confidence_threshold_indices],
                                                         self.keep_prob: self.dropout_prob})
                        else:
    ...
    def test(self, test_batch, test_labels, counter=0, is_arg_max=False):
        if is_arg_max:
            ...
            print('step {}: accuracy:{}, mean_confidence:{}, loss:{}'.format(counter, accuracy, np.mean(confidence),
                                                                             loss))
            ...
            accuracy, confidence, loss = self.sess.run([self.accuracy, self.confidence, self.cross_entropy],
                                                       feed_dict={self.x: test_batch.reshape(-1, 784),
                                                                  self.y_: test_labels, self.keep_prob: 1.})
            # self.test_writer.add_summary(summary, counter)
    ...
    def save_model(self):
        ...
        pickle.dump([self.sess.run(self.W_conv1), self.sess.run(self.b_conv1), self.sess.run(self.W_conv2),
                     self.sess.run(self.b_conv2), self.sess.run(self.W_fc1),
                     self.sess.run(self.b_fc1), self.sess.run(self.W_fc2), self.sess.run(self.b_fc2)],
    ...
    def plot_train_test_loss(self, name_of_measure, array, color="b", marker="P",
                             dir="classifier_results_seed_{}/".format(SEED)):
