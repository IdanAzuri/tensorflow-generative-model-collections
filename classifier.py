	...
	def __init__(self, classifier_name, load_from_pkl=False, pkl_path=None, pkl_label_path=None,do_preprocess=False):
		...
		if load_from_pkl:
			...
			if do_preprocess:
				...
				pickle.dump(self.data_y, open("{}/edited_{}".format(fname_labels[0],fname_labels[1]), 'wb'))
				pickle.dump(self.data_X, open("{}/edited_{}".format(fname_images[0],fname_images[1]), 'wb'))
				np.random.seed(seed)
