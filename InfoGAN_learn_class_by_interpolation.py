...
class MultiModalInfoGAN_phase2(object):
	...
	def __init__(self, sess, epoch, batch_size, z_dim, dataset_name, checkpoint_dir, result_dir, log_dir, sampler, seed, len_continuous_code=2, is_wgan_gp=False,
		...
		if dataset_name == 'mnist' or dataset_name == 'fashion-mnist':
			...
			self.n = 512  # for 2 random indices
	...
	def train(self):
		...
		for epoch in range(start_epoch, self.epoch):
			...
			for idx in range(start_batch_id, max(self.num_batches, 1)):
				...
				batch_labels = [2.91529568e-01, 5.47097897e-01, 9.21683068e-02, 1.60650195e-02, 7.58631531e-03, 3.82596212e-02, 6.68696731e-04, 8.30287637e-04, 3.12111602e-03]* self.batch_size
				np.asarray(batch_labels).reshape(-1,self.len_discrete_code)
				print("Simplex:{}".format(batch_labels.shape))
