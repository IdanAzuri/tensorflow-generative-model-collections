...
def fashion_MM_plot_from_pkl():
	...
	d = np.mean(
		pickle.load(open(dir + "classifier_MMinfoGAN_fashion-mnist_MultiModalUniformSample_mu_0.0_sigma_0.1_ndist_10_accuracy_cv_3_gan_only_no_prior.pkl", "rb"))[START:END])
...
def MMgeneral_plot_from_pkl(groupby=""):
	...
	for dir in dirs:
		for f in glob.iglob("{}/*{}*.pkl".format(dir,groupby)):
			...
			files_list[fname].append(np.max(pickle.load(open(f, "rb"))))
	...
	title = 'MMinfoGAN_Fsion-Mnist_multi-modal Multi modal Gaussian'
...
if __name__ == '__main__':
	...
	MMgeneral_plot_from_pkl("ndist_10")
